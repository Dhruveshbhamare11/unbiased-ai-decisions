...
def apply_reweighing_and_retrain(df_work_in: pd.DataFrame,
    ...
    try:
        ...
        logger.error(f'AIF360 PIPELINE CRASHED AT STEP: {e}')
...
async def debias_csv(file: UploadFile = File(...)):
    ...
    try:
        logger.info(f'Starting AIF360 pipeline')
        logger.info(f'df shape: {df_clean.shape}')
        logger.info(f'columns: {list(df_clean.columns)}')
        logger.info(f'label_col: {label_col}, sensitive_col: {primary_sensitive}')
        logger.info(f'privileged_val: {orig_priv_val}')
        logger.info(f'label value counts: {df_clean[label_col].value_counts().to_dict()}')
        if primary_sensitive in df_clean.columns:
            logger.info(f'sensitive value counts: {df_clean[primary_sensitive].value_counts().to_dict()}')
